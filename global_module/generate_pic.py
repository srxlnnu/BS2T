...
def generate_iterTrans(TRAIN_SIZE, train_indices, TEST_SIZE, test_indices, TOTAL_SIZE, total_indices, VAL_SIZE,
                  whole_data, PATCH_LENGTH, padded_data, PATCH_LENGTH1, padded_data1, INPUT_DIMENSION, batch_size, gt):
    ...
    train_data = extract_samll_cubic.select_small_cubic(TRAIN_SIZE, train_indices, whole_data,
                                                        PATCH_LENGTH, padded_data, INPUT_DIMENSION)
    train_data_spe = extract_samll_cubic.select_small_cubic(TRAIN_SIZE, train_indices, whole_data,
                                                        PATCH_LENGTH1, padded_data1, INPUT_DIMENSION)
    all_data = extract_samll_cubic.select_small_cubic(TOTAL_SIZE, total_indices, whole_data,
                                                      PATCH_LENGTH, padded_data, INPUT_DIMENSION)
    all_data_spe = extract_samll_cubic.select_small_cubic(TOTAL_SIZE, total_indices, whole_data,
                                                          PATCH_LENGTH1, padded_data1, INPUT_DIMENSION)
    test_data = extract_samll_cubic.select_small_cubic(TEST_SIZE, test_indices, whole_data,
                                                       PATCH_LENGTH, padded_data, INPUT_DIMENSION)
    test_data_spe = extract_samll_cubic.select_small_cubic(TEST_SIZE, test_indices, whole_data,
                                                       PATCH_LENGTH1, padded_data1, INPUT_DIMENSION)

    x_train = train_data.reshape(train_data.shape[0], train_data.shape[1], train_data.shape[2], INPUT_DIMENSION)
    x_train_spe = train_data_spe.reshape(train_data_spe.shape[0], train_data_spe.shape[1], train_data_spe.shape[2], INPUT_DIMENSION)
    x_test_all = test_data.reshape(test_data.shape[0], test_data.shape[1], test_data.shape[2], INPUT_DIMENSION)
    x_test_all_spe = test_data_spe.reshape(test_data_spe.shape[0], test_data_spe.shape[1], test_data_spe.shape[2], INPUT_DIMENSION)
    x_all = all_data.reshape(all_data.shape[0], all_data.shape[1], all_data.shape[2], INPUT_DIMENSION)
    x_all_spe = all_data_spe.reshape(all_data_spe.shape[0], all_data_spe.shape[1], all_data_spe.shape[2], INPUT_DIMENSION)


    x_val = x_test_all[-VAL_SIZE:]
    x_val_spe = x_test_all_spe[-VAL_SIZE:]
    y_val = y_test[-VAL_SIZE:]
