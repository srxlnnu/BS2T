...
for index_iter in range(ITER):
    ...
    with torch.no_grad():
        for X, y in test_iter:
            ...
            y_hat = newnet(X)
            # print(net(X))
            pred_test_fdssc.extend(np.array(newnet(X).cpu().argmax(axis=1)))
    toc2 = time.clock()
    collections.Counter(pred_test_fdssc)
    gt_test1 = gt[test_indices] - 1
    gt_test = gt_test1[:-VAL_SIZE]
    sio.savemat('UP_pred_proposed.mat', {'pred':pred_test_fdssc})
    sio.savemat('UP_gt_proposed.mat', {'gt': gt_test})
    ...
    torch.save(newnet.state_dict(), "./net_DBA" + str(round(overall_acc_fdssc, 3)) + '.pt')
    KAPPA.append(kappa)
    ...
    print('AA:',AA)
    print('Kappa:',KAPPA)
# np.record(OA, AA, KAPPA, ELEMENT_ACC, TRAINING_TIME, TESTING_TIME,
#                      'records/' + net.name + day_str + '_' + Dataset + 'split：' + str(VALIDATION_SPLIT) + 'lr：' + str(lr) + '.txt')


    generate_png(all_iter, newnet, gt_hsi, 'UP', device, total_indices)
